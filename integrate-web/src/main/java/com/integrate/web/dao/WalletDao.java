...
public class WalletDao {
    ...
    public long addCouponExchange(long userId,
                                  int integrate,
                                  int type,
                                  String account,
                                  long couponId,
                                  int amount) {
        long time = System.currentTimeMillis();
        String sql = "INSERT INTO t_coupon_exchange (user_id, integrate, type, account, coupon_id, amount, status, create_time, update_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);";
        return jdbc.insert(sql, "id", userId, integrate, type, account, couponId, amount, 0, time, time);
    }
