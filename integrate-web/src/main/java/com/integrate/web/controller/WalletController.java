...
public class WalletController {
    ...
    public void exchange2(HttpServletRequest request,HttpServletResponse response) {
        ...
        int integrate = product.getIntegrate() * Integer.parseInt(amount);
        if (product.getIntegrate() * Integer.parseInt(amount) > walletService.userintegrate(Long.parseLong(userId))) {
            Message.writeError(response, SysMsgEnumType.INTEGRATE_FAIL);
            return;
        }
        boolean result = walletService.addRechrageRecorde2(Long.valueOf(userId), integrate, mobile, userName, address, Long.valueOf(productId), Integer.parseInt(amount));
        if (!result) {
            Message.writeError(response, SysMsgEnumType.TRADE_FAIL);
            return;
        }
